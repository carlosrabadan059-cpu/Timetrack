# Graph Report - Timetrack  (2026-07-01)

## Corpus Check
- 250 files · ~197,342 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3563 nodes · 3692 edges · 383 communities (364 shown, 19 thin omitted)
- Extraction: 100% EXTRACTED · 0% INFERRED · 0% AMBIGUOUS
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `729fbcc8`
- Run `git rev-parse HEAD` and compare to check if the graph is stale.
- Run `graphify update .` after code changes (no API cost).

## Community Hubs (Navigation)
- [[_COMMUNITY_Community 0|Community 0]]
- [[_COMMUNITY_Community 1|Community 1]]
- [[_COMMUNITY_Community 2|Community 2]]
- [[_COMMUNITY_Community 3|Community 3]]
- [[_COMMUNITY_Community 4|Community 4]]
- [[_COMMUNITY_Community 5|Community 5]]
- [[_COMMUNITY_Community 6|Community 6]]
- [[_COMMUNITY_Community 7|Community 7]]
- [[_COMMUNITY_Community 8|Community 8]]
- [[_COMMUNITY_Community 9|Community 9]]
- [[_COMMUNITY_Community 10|Community 10]]
- [[_COMMUNITY_Community 11|Community 11]]
- [[_COMMUNITY_Community 12|Community 12]]
- [[_COMMUNITY_Community 13|Community 13]]
- [[_COMMUNITY_Community 14|Community 14]]
- [[_COMMUNITY_Community 15|Community 15]]
- [[_COMMUNITY_Community 16|Community 16]]
- [[_COMMUNITY_Community 17|Community 17]]
- [[_COMMUNITY_Community 18|Community 18]]
- [[_COMMUNITY_Community 19|Community 19]]
- [[_COMMUNITY_Community 20|Community 20]]
- [[_COMMUNITY_Community 21|Community 21]]
- [[_COMMUNITY_Community 22|Community 22]]
- [[_COMMUNITY_Community 23|Community 23]]
- [[_COMMUNITY_Community 24|Community 24]]
- [[_COMMUNITY_Community 25|Community 25]]
- [[_COMMUNITY_Community 26|Community 26]]
- [[_COMMUNITY_Community 27|Community 27]]
- [[_COMMUNITY_Community 28|Community 28]]
- [[_COMMUNITY_Community 29|Community 29]]
- [[_COMMUNITY_Community 30|Community 30]]
- [[_COMMUNITY_Community 31|Community 31]]
- [[_COMMUNITY_Community 32|Community 32]]
- [[_COMMUNITY_Community 33|Community 33]]
- [[_COMMUNITY_Community 34|Community 34]]
- [[_COMMUNITY_Community 35|Community 35]]
- [[_COMMUNITY_Community 36|Community 36]]
- [[_COMMUNITY_Community 37|Community 37]]
- [[_COMMUNITY_Community 38|Community 38]]
- [[_COMMUNITY_Community 40|Community 40]]
- [[_COMMUNITY_Community 41|Community 41]]
- [[_COMMUNITY_Community 42|Community 42]]
- [[_COMMUNITY_Community 43|Community 43]]
- [[_COMMUNITY_Community 44|Community 44]]
- [[_COMMUNITY_Community 45|Community 45]]
- [[_COMMUNITY_Community 46|Community 46]]
- [[_COMMUNITY_Community 47|Community 47]]
- [[_COMMUNITY_Community 48|Community 48]]
- [[_COMMUNITY_Community 49|Community 49]]
- [[_COMMUNITY_Community 50|Community 50]]
- [[_COMMUNITY_Community 51|Community 51]]
- [[_COMMUNITY_Community 52|Community 52]]
- [[_COMMUNITY_Community 53|Community 53]]
- [[_COMMUNITY_Community 54|Community 54]]
- [[_COMMUNITY_Community 55|Community 55]]
- [[_COMMUNITY_Community 56|Community 56]]
- [[_COMMUNITY_Community 57|Community 57]]
- [[_COMMUNITY_Community 58|Community 58]]
- [[_COMMUNITY_Community 59|Community 59]]
- [[_COMMUNITY_Community 60|Community 60]]
- [[_COMMUNITY_Community 61|Community 61]]
- [[_COMMUNITY_Community 62|Community 62]]
- [[_COMMUNITY_Community 63|Community 63]]
- [[_COMMUNITY_Community 64|Community 64]]
- [[_COMMUNITY_Community 65|Community 65]]
- [[_COMMUNITY_Community 66|Community 66]]
- [[_COMMUNITY_Community 67|Community 67]]
- [[_COMMUNITY_Community 68|Community 68]]
- [[_COMMUNITY_Community 69|Community 69]]
- [[_COMMUNITY_Community 70|Community 70]]
- [[_COMMUNITY_Community 71|Community 71]]
- [[_COMMUNITY_Community 72|Community 72]]
- [[_COMMUNITY_Community 73|Community 73]]
- [[_COMMUNITY_Community 74|Community 74]]
- [[_COMMUNITY_Community 75|Community 75]]
- [[_COMMUNITY_Community 76|Community 76]]
- [[_COMMUNITY_Community 77|Community 77]]
- [[_COMMUNITY_Community 78|Community 78]]
- [[_COMMUNITY_Community 79|Community 79]]
- [[_COMMUNITY_Community 80|Community 80]]
- [[_COMMUNITY_Community 81|Community 81]]
- [[_COMMUNITY_Community 82|Community 82]]
- [[_COMMUNITY_Community 83|Community 83]]
- [[_COMMUNITY_Community 84|Community 84]]
- [[_COMMUNITY_Community 85|Community 85]]
- [[_COMMUNITY_Community 86|Community 86]]
- [[_COMMUNITY_Community 87|Community 87]]
- [[_COMMUNITY_Community 88|Community 88]]
- [[_COMMUNITY_Community 89|Community 89]]
- [[_COMMUNITY_Community 90|Community 90]]
- [[_COMMUNITY_Community 91|Community 91]]
- [[_COMMUNITY_Community 92|Community 92]]
- [[_COMMUNITY_Community 93|Community 93]]
- [[_COMMUNITY_Community 94|Community 94]]
- [[_COMMUNITY_Community 95|Community 95]]
- [[_COMMUNITY_Community 96|Community 96]]
- [[_COMMUNITY_Community 97|Community 97]]
- [[_COMMUNITY_Community 98|Community 98]]
- [[_COMMUNITY_Community 99|Community 99]]
- [[_COMMUNITY_Community 100|Community 100]]
- [[_COMMUNITY_Community 101|Community 101]]
- [[_COMMUNITY_Community 102|Community 102]]
- [[_COMMUNITY_Community 103|Community 103]]
- [[_COMMUNITY_Community 104|Community 104]]
- [[_COMMUNITY_Community 105|Community 105]]
- [[_COMMUNITY_Community 106|Community 106]]
- [[_COMMUNITY_Community 107|Community 107]]
- [[_COMMUNITY_Community 108|Community 108]]
- [[_COMMUNITY_Community 109|Community 109]]
- [[_COMMUNITY_Community 110|Community 110]]
- [[_COMMUNITY_Community 111|Community 111]]
- [[_COMMUNITY_Community 112|Community 112]]
- [[_COMMUNITY_Community 113|Community 113]]
- [[_COMMUNITY_Community 114|Community 114]]
- [[_COMMUNITY_Community 115|Community 115]]
- [[_COMMUNITY_Community 116|Community 116]]
- [[_COMMUNITY_Community 117|Community 117]]
- [[_COMMUNITY_Community 118|Community 118]]
- [[_COMMUNITY_Community 119|Community 119]]
- [[_COMMUNITY_Community 120|Community 120]]
- [[_COMMUNITY_Community 121|Community 121]]
- [[_COMMUNITY_Community 122|Community 122]]
- [[_COMMUNITY_Community 123|Community 123]]
- [[_COMMUNITY_Community 124|Community 124]]
- [[_COMMUNITY_Community 125|Community 125]]
- [[_COMMUNITY_Community 126|Community 126]]
- [[_COMMUNITY_Community 127|Community 127]]
- [[_COMMUNITY_Community 128|Community 128]]
- [[_COMMUNITY_Community 129|Community 129]]
- [[_COMMUNITY_Community 130|Community 130]]
- [[_COMMUNITY_Community 131|Community 131]]
- [[_COMMUNITY_Community 132|Community 132]]
- [[_COMMUNITY_Community 133|Community 133]]
- [[_COMMUNITY_Community 134|Community 134]]
- [[_COMMUNITY_Community 135|Community 135]]
- [[_COMMUNITY_Community 136|Community 136]]
- [[_COMMUNITY_Community 137|Community 137]]
- [[_COMMUNITY_Community 138|Community 138]]
- [[_COMMUNITY_Community 139|Community 139]]
- [[_COMMUNITY_Community 140|Community 140]]
- [[_COMMUNITY_Community 141|Community 141]]
- [[_COMMUNITY_Community 142|Community 142]]
- [[_COMMUNITY_Community 143|Community 143]]
- [[_COMMUNITY_Community 144|Community 144]]
- [[_COMMUNITY_Community 145|Community 145]]
- [[_COMMUNITY_Community 146|Community 146]]
- [[_COMMUNITY_Community 147|Community 147]]
- [[_COMMUNITY_Community 148|Community 148]]
- [[_COMMUNITY_Community 149|Community 149]]
- [[_COMMUNITY_Community 150|Community 150]]
- [[_COMMUNITY_Community 151|Community 151]]
- [[_COMMUNITY_Community 152|Community 152]]
- [[_COMMUNITY_Community 153|Community 153]]
- [[_COMMUNITY_Community 154|Community 154]]
- [[_COMMUNITY_Community 155|Community 155]]
- [[_COMMUNITY_Community 156|Community 156]]
- [[_COMMUNITY_Community 157|Community 157]]
- [[_COMMUNITY_Community 158|Community 158]]
- [[_COMMUNITY_Community 159|Community 159]]
- [[_COMMUNITY_Community 160|Community 160]]
- [[_COMMUNITY_Community 161|Community 161]]
- [[_COMMUNITY_Community 162|Community 162]]
- [[_COMMUNITY_Community 163|Community 163]]
- [[_COMMUNITY_Community 164|Community 164]]
- [[_COMMUNITY_Community 165|Community 165]]
- [[_COMMUNITY_Community 166|Community 166]]
- [[_COMMUNITY_Community 167|Community 167]]
- [[_COMMUNITY_Community 168|Community 168]]
- [[_COMMUNITY_Community 169|Community 169]]
- [[_COMMUNITY_Community 170|Community 170]]
- [[_COMMUNITY_Community 171|Community 171]]
- [[_COMMUNITY_Community 172|Community 172]]
- [[_COMMUNITY_Community 173|Community 173]]
- [[_COMMUNITY_Community 174|Community 174]]
- [[_COMMUNITY_Community 175|Community 175]]
- [[_COMMUNITY_Community 176|Community 176]]
- [[_COMMUNITY_Community 177|Community 177]]
- [[_COMMUNITY_Community 178|Community 178]]
- [[_COMMUNITY_Community 179|Community 179]]
- [[_COMMUNITY_Community 180|Community 180]]
- [[_COMMUNITY_Community 181|Community 181]]
- [[_COMMUNITY_Community 182|Community 182]]
- [[_COMMUNITY_Community 183|Community 183]]
- [[_COMMUNITY_Community 184|Community 184]]
- [[_COMMUNITY_Community 185|Community 185]]
- [[_COMMUNITY_Community 186|Community 186]]
- [[_COMMUNITY_Community 187|Community 187]]
- [[_COMMUNITY_Community 188|Community 188]]
- [[_COMMUNITY_Community 189|Community 189]]
- [[_COMMUNITY_Community 190|Community 190]]
- [[_COMMUNITY_Community 191|Community 191]]
- [[_COMMUNITY_Community 192|Community 192]]
- [[_COMMUNITY_Community 193|Community 193]]
- [[_COMMUNITY_Community 194|Community 194]]
- [[_COMMUNITY_Community 195|Community 195]]
- [[_COMMUNITY_Community 196|Community 196]]
- [[_COMMUNITY_Community 197|Community 197]]
- [[_COMMUNITY_Community 198|Community 198]]
- [[_COMMUNITY_Community 199|Community 199]]
- [[_COMMUNITY_Community 200|Community 200]]
- [[_COMMUNITY_Community 201|Community 201]]
- [[_COMMUNITY_Community 202|Community 202]]
- [[_COMMUNITY_Community 203|Community 203]]
- [[_COMMUNITY_Community 204|Community 204]]
- [[_COMMUNITY_Community 205|Community 205]]
- [[_COMMUNITY_Community 206|Community 206]]
- [[_COMMUNITY_Community 207|Community 207]]
- [[_COMMUNITY_Community 208|Community 208]]
- [[_COMMUNITY_Community 209|Community 209]]
- [[_COMMUNITY_Community 210|Community 210]]
- [[_COMMUNITY_Community 211|Community 211]]
- [[_COMMUNITY_Community 212|Community 212]]
- [[_COMMUNITY_Community 213|Community 213]]
- [[_COMMUNITY_Community 214|Community 214]]
- [[_COMMUNITY_Community 215|Community 215]]
- [[_COMMUNITY_Community 216|Community 216]]
- [[_COMMUNITY_Community 217|Community 217]]
- [[_COMMUNITY_Community 218|Community 218]]
- [[_COMMUNITY_Community 219|Community 219]]
- [[_COMMUNITY_Community 220|Community 220]]
- [[_COMMUNITY_Community 221|Community 221]]
- [[_COMMUNITY_Community 222|Community 222]]
- [[_COMMUNITY_Community 223|Community 223]]
- [[_COMMUNITY_Community 224|Community 224]]
- [[_COMMUNITY_Community 225|Community 225]]
- [[_COMMUNITY_Community 226|Community 226]]
- [[_COMMUNITY_Community 227|Community 227]]
- [[_COMMUNITY_Community 228|Community 228]]
- [[_COMMUNITY_Community 229|Community 229]]
- [[_COMMUNITY_Community 230|Community 230]]
- [[_COMMUNITY_Community 231|Community 231]]
- [[_COMMUNITY_Community 232|Community 232]]
- [[_COMMUNITY_Community 233|Community 233]]
- [[_COMMUNITY_Community 234|Community 234]]
- [[_COMMUNITY_Community 235|Community 235]]
- [[_COMMUNITY_Community 236|Community 236]]
- [[_COMMUNITY_Community 237|Community 237]]
- [[_COMMUNITY_Community 238|Community 238]]
- [[_COMMUNITY_Community 239|Community 239]]
- [[_COMMUNITY_Community 240|Community 240]]
- [[_COMMUNITY_Community 241|Community 241]]
- [[_COMMUNITY_Community 242|Community 242]]
- [[_COMMUNITY_Community 243|Community 243]]
- [[_COMMUNITY_Community 244|Community 244]]
- [[_COMMUNITY_Community 245|Community 245]]
- [[_COMMUNITY_Community 246|Community 246]]
- [[_COMMUNITY_Community 247|Community 247]]
- [[_COMMUNITY_Community 248|Community 248]]
- [[_COMMUNITY_Community 249|Community 249]]
- [[_COMMUNITY_Community 250|Community 250]]
- [[_COMMUNITY_Community 251|Community 251]]
- [[_COMMUNITY_Community 252|Community 252]]
- [[_COMMUNITY_Community 253|Community 253]]
- [[_COMMUNITY_Community 254|Community 254]]
- [[_COMMUNITY_Community 255|Community 255]]
- [[_COMMUNITY_Community 256|Community 256]]
- [[_COMMUNITY_Community 257|Community 257]]
- [[_COMMUNITY_Community 258|Community 258]]
- [[_COMMUNITY_Community 259|Community 259]]
- [[_COMMUNITY_Community 260|Community 260]]
- [[_COMMUNITY_Community 261|Community 261]]
- [[_COMMUNITY_Community 262|Community 262]]
- [[_COMMUNITY_Community 263|Community 263]]
- [[_COMMUNITY_Community 264|Community 264]]
- [[_COMMUNITY_Community 265|Community 265]]
- [[_COMMUNITY_Community 266|Community 266]]
- [[_COMMUNITY_Community 267|Community 267]]
- [[_COMMUNITY_Community 268|Community 268]]
- [[_COMMUNITY_Community 269|Community 269]]
- [[_COMMUNITY_Community 270|Community 270]]
- [[_COMMUNITY_Community 271|Community 271]]
- [[_COMMUNITY_Community 272|Community 272]]
- [[_COMMUNITY_Community 273|Community 273]]
- [[_COMMUNITY_Community 274|Community 274]]
- [[_COMMUNITY_Community 275|Community 275]]
- [[_COMMUNITY_Community 276|Community 276]]
- [[_COMMUNITY_Community 277|Community 277]]
- [[_COMMUNITY_Community 278|Community 278]]
- [[_COMMUNITY_Community 279|Community 279]]
- [[_COMMUNITY_Community 280|Community 280]]
- [[_COMMUNITY_Community 281|Community 281]]
- [[_COMMUNITY_Community 282|Community 282]]
- [[_COMMUNITY_Community 283|Community 283]]
- [[_COMMUNITY_Community 284|Community 284]]
- [[_COMMUNITY_Community 285|Community 285]]
- [[_COMMUNITY_Community 286|Community 286]]
- [[_COMMUNITY_Community 287|Community 287]]
- [[_COMMUNITY_Community 288|Community 288]]
- [[_COMMUNITY_Community 289|Community 289]]
- [[_COMMUNITY_Community 290|Community 290]]
- [[_COMMUNITY_Community 291|Community 291]]
- [[_COMMUNITY_Community 292|Community 292]]
- [[_COMMUNITY_Community 293|Community 293]]
- [[_COMMUNITY_Community 294|Community 294]]
- [[_COMMUNITY_Community 295|Community 295]]
- [[_COMMUNITY_Community 296|Community 296]]
- [[_COMMUNITY_Community 297|Community 297]]
- [[_COMMUNITY_Community 298|Community 298]]
- [[_COMMUNITY_Community 299|Community 299]]
- [[_COMMUNITY_Community 300|Community 300]]
- [[_COMMUNITY_Community 301|Community 301]]
- [[_COMMUNITY_Community 302|Community 302]]
- [[_COMMUNITY_Community 303|Community 303]]
- [[_COMMUNITY_Community 304|Community 304]]
- [[_COMMUNITY_Community 305|Community 305]]
- [[_COMMUNITY_Community 306|Community 306]]
- [[_COMMUNITY_Community 307|Community 307]]
- [[_COMMUNITY_Community 308|Community 308]]
- [[_COMMUNITY_Community 309|Community 309]]
- [[_COMMUNITY_Community 310|Community 310]]
- [[_COMMUNITY_Community 311|Community 311]]
- [[_COMMUNITY_Community 312|Community 312]]
- [[_COMMUNITY_Community 313|Community 313]]
- [[_COMMUNITY_Community 314|Community 314]]
- [[_COMMUNITY_Community 315|Community 315]]
- [[_COMMUNITY_Community 316|Community 316]]
- [[_COMMUNITY_Community 317|Community 317]]
- [[_COMMUNITY_Community 318|Community 318]]
- [[_COMMUNITY_Community 319|Community 319]]
- [[_COMMUNITY_Community 320|Community 320]]
- [[_COMMUNITY_Community 321|Community 321]]
- [[_COMMUNITY_Community 322|Community 322]]
- [[_COMMUNITY_Community 323|Community 323]]
- [[_COMMUNITY_Community 324|Community 324]]
- [[_COMMUNITY_Community 325|Community 325]]
- [[_COMMUNITY_Community 326|Community 326]]
- [[_COMMUNITY_Community 327|Community 327]]
- [[_COMMUNITY_Community 328|Community 328]]
- [[_COMMUNITY_Community 330|Community 330]]
- [[_COMMUNITY_Community 331|Community 331]]
- [[_COMMUNITY_Community 332|Community 332]]
- [[_COMMUNITY_Community 333|Community 333]]
- [[_COMMUNITY_Community 334|Community 334]]
- [[_COMMUNITY_Community 335|Community 335]]
- [[_COMMUNITY_Community 336|Community 336]]
- [[_COMMUNITY_Community 337|Community 337]]
- [[_COMMUNITY_Community 338|Community 338]]
- [[_COMMUNITY_Community 339|Community 339]]
- [[_COMMUNITY_Community 340|Community 340]]
- [[_COMMUNITY_Community 341|Community 341]]
- [[_COMMUNITY_Community 342|Community 342]]
- [[_COMMUNITY_Community 343|Community 343]]
- [[_COMMUNITY_Community 344|Community 344]]
- [[_COMMUNITY_Community 345|Community 345]]
- [[_COMMUNITY_Community 346|Community 346]]
- [[_COMMUNITY_Community 347|Community 347]]
- [[_COMMUNITY_Community 348|Community 348]]
- [[_COMMUNITY_Community 349|Community 349]]
- [[_COMMUNITY_Community 350|Community 350]]
- [[_COMMUNITY_Community 351|Community 351]]
- [[_COMMUNITY_Community 352|Community 352]]
- [[_COMMUNITY_Community 353|Community 353]]
- [[_COMMUNITY_Community 354|Community 354]]
- [[_COMMUNITY_Community 355|Community 355]]
- [[_COMMUNITY_Community 356|Community 356]]
- [[_COMMUNITY_Community 357|Community 357]]
- [[_COMMUNITY_Community 358|Community 358]]
- [[_COMMUNITY_Community 362|Community 362]]
- [[_COMMUNITY_Community 364|Community 364]]
- [[_COMMUNITY_Community 365|Community 365]]
- [[_COMMUNITY_Community 366|Community 366]]
- [[_COMMUNITY_Community 367|Community 367]]
- [[_COMMUNITY_Community 368|Community 368]]
- [[_COMMUNITY_Community 369|Community 369]]
- [[_COMMUNITY_Community 370|Community 370]]
- [[_COMMUNITY_Community 381|Community 381]]
- [[_COMMUNITY_Community 382|Community 382]]

## God Nodes (most connected - your core abstractions)
1. `getSupabaseAdmin()` - 34 edges
2. `skills` - 22 edges
3. `n8n MCP Tools Expert` - 19 edges
4. `AI Agent Workflow Pattern` - 19 edges
5. `compilerOptions` - 18 edges
6. `expo` - 18 edges
7. `Python Code Node (Beta)` - 18 edges
8. `Common n8n Expression Mistakes` - 18 edges
9. `Common Patterns - Python Code Node` - 17 edges
10. `Workflow Management Tools Guide` - 17 edges

## Surprising Connections (you probably didn't know these)
- `main()` --calls--> `getSupabaseAdmin()`  [EXTRACTED]
  backend/scripts/check-carlos-profile.ts → backend/src/lib/supabase.ts
- `main()` --calls--> `getSupabaseAdmin()`  [EXTRACTED]
  backend/scripts/create-test-api-key.ts → backend/src/lib/supabase.ts
- `main()` --calls--> `getSupabaseAdmin()`  [EXTRACTED]
  backend/scripts/generate-report-demo.ts → backend/src/lib/supabase.ts
- `main()` --calls--> `getSupabaseAdmin()`  [EXTRACTED]
  backend/scripts/seed-carlos-logs.ts → backend/src/lib/supabase.ts
- `main()` --calls--> `getSupabaseAdmin()`  [EXTRACTED]
  backend/scripts/update-carlos-company.ts → backend/src/lib/supabase.ts

## Import Cycles
- None detected.

## Communities (383 total, 19 thin omitted)

### Community 0 - "Community 0"
Cohesion: 0.04
Nodes (48): 1. Always Use {{}}, 2. Use Quotes for Spaces and Special Characters, 3. Match Exact Node Names, 4. No Nested {{}}, Access Nested Fields, Advanced Patterns, Arrays, Available Methods (+40 more)

### Community 1 - "Community 1"
Cohesion: 0.04
Nodes (46): 1. Use .get() for Safe Access, 2. Handle Empty Lists, 3. Use List Comprehensions, 4. Return Proper Format, 5. Use Standard Library, Array Operations, Best Practices, Common Patterns - Python Code Node (+38 more)

### Community 2 - "Community 2"
Cohesion: 0.04
Nodes (45): 0. Sandbox Restrictions (Critical), 1. $helpers.httpRequest() - HTTP Requests, 2. DateTime (Luxon) - Date & Time Operations, 3. $jmespath() - JSON Querying, 4. $getWorkflowStaticData() - Persistent Storage, 5. Standard JavaScript Globals, 6. Available Node.js Modules, Advanced Queries (+37 more)

### Community 3 - "Community 3"
Cohesion: 0.04
Nodes (45): Advanced Patterns, Basic Usage, Basic Usage, Basic Usage, Basic Usage, Choosing the Right Pattern, Common Mistakes, Critical: Webhook Data Structure (+37 more)

### Community 4 - "Community 4"
Cohesion: 0.04
Nodes (45): Backend (`backend/package.json`) — npm audit summary, [CN-001] jsPDF ≤4.2.0 — PDF Injection, XSS, Path Traversal, and DoS Cluster, [CN-002] Missing Security Headers on Vercel Frontend Deployment, [CN-003] Docker Container Runs as Root — No USER Directive, [CN-004] Missing .dockerignore — Backend .env Copied into Docker Image Layer, [CN-005] AC API Token Stored as Plaintext in Supabase Database, [CN-006] SSRF — Admin Can Probe Internal LAN via Test-AC Endpoint, [CN-007] xlsx@0.18.5 — Prototype Pollution + ReDoS, No npm Fix Available (+37 more)

### Community 5 - "Community 5"
Cohesion: 0.04
Nodes (44): All Emails (Join), Array Length, Bracket Notation, Combined, Custom Date Format, Date Math - Future, Date Math - Past, Default Values (+36 more)

### Community 6 - "Community 6"
Cohesion: 0.04
Nodes (44): 1. missing_required, 2. invalid_value, 3. type_mismatch, 4. invalid_expression, 5. invalid_reference, 6. best_practice, 7. deprecated, 8. performance (+36 more)

### Community 7 - "Community 7"
Cohesion: 0.07
Nodes (33): checkNonWorkingDay(), checkOutOfSchedule(), DAYS_ES, DAYS_FULL_ES, formatDateLabel(), getWeekStart(), MONTHS_ES, monthYearLabel() (+25 more)

### Community 8 - "Community 8"
Cohesion: 0.05
Nodes (42): dependencies, expo, expo-constants, expo-linking, expo-location, @expo/metro-runtime, expo-router, expo-secure-store (+34 more)

### Community 9 - "Community 9"
Cohesion: 0.05
Nodes (41): Best Practices, COMMON_PATTERNS.md (895 lines), Common Use Cases, Core Concepts, Credits, Data Access, DATA_ACCESS.md (703 lines), ERROR_PATTERNS.md (730 lines) (+33 more)

### Community 10 - "Community 10"
Cohesion: 0.05
Nodes (40): Advanced Patterns, Auto-Sanitization and Dependencies, Basic Format, Best Practices, Common Dependency Patterns, Complex Dependency Examples, displayOptions Structure, ✅ Do (+32 more)

### Community 11 - "Community 11"
Cohesion: 0.05
Nodes (40): dependencies, date-fns, jspdf, jspdf-autotable, leaflet, lucide-react, react, react-dom (+32 more)

### Community 12 - "Community 12"
Cohesion: 0.05
Nodes (39): 1. Missing Error Handling, 2. No Retry Logic, 3. Missing Rate Limiting, 4. Unbounded Database Queries, 5. Missing Input Validation, 6. Hardcoded Credentials, Always Acceptable, Always Fix (+31 more)

### Community 13 - "Community 13"
Cohesion: 0.05
Nodes (37): 1. Be Specific, 1. n8n Expression Syntax, 2. Ask Follow-Up Questions, 2. n8n MCP Tools Expert, 3. n8n Workflow Patterns, 3. Request Validation, 4. Leverage Cross-Skill Knowledge, 4. n8n Validation Expert (+29 more)

### Community 14 - "Community 14"
Cohesion: 0.05
Nodes (37): Available Standard Library Modules, Best Practice, Bonus Error: AttributeError, Common Library Replacements, Common Scenarios, Error #1: ModuleNotFoundError (MOST CRITICAL), Error #2: Empty Code / Missing Return, Error #3: KeyError (+29 more)

### Community 15 - "Community 15"
Cohesion: 0.05
Nodes (36): backgroundColor, foregroundImage, adaptiveIcon, edgeToEdgeEnabled, package, predictiveBackGestureEnabled, typedRoutes, expo (+28 more)

### Community 16 - "Community 16"
Cohesion: 0.05
Nodes (36): Choosing the Right Pattern, Combining Patterns, Common Patterns - JavaScript Code Node, Complete Example, Complete Example, Complete Example, Complete Example, Complete Example (+28 more)

### Community 17 - "Community 17"
Cohesion: 0.05
Nodes (36): #1: Webhook Data Structure, #2: Return Format, #3: Expression Syntax, Activates On, Author, Built-in Functions, Built-in Functions, Common Patterns (Production-Tested) (+28 more)

### Community 18 - "Community 18"
Cohesion: 0.06
Nodes (32): 1. n8n Expression Syntax, 2. n8n MCP Tools Expert (HIGHEST PRIORITY), 3. n8n Workflow Patterns, 4. n8n Validation Expert, 5. n8n Node Configuration, 6. n8n Code JavaScript, 7. n8n Code Python, Credential Management (+24 more)

### Community 19 - "Community 19"
Cohesion: 0.08
Nodes (16): requireRole(), adminIncidencias, createIncidenciaSchema, HookResult, incidencias, resolveSchema, adminVacaciones, createVacacionSchema (+8 more)

### Community 20 - "Community 20"
Cohesion: 0.10
Nodes (20): apiKeyAuthMiddleware, auth, docs, spec, external, FichajeItem, fichajesQuerySchema, fmtTs() (+12 more)

### Community 21 - "Community 21"
Cohesion: 0.07
Nodes (28): 1. Progressive Disclosure Works, 2. Operations Determine Requirements, 3. Dependencies Control Visibility, 4. Configuration is Iterative, 5. Common Gotchas Exist, Author, Complementary:, Core Concepts (+20 more)

### Community 22 - "Community 22"
Cohesion: 0.07
Nodes (27): 1. **n8n Expression Syntax**, 2. **n8n MCP Tools Expert** (HIGHEST PRIORITY), 3. **n8n Workflow Patterns**, 4. **n8n Validation Expert**, 5. **n8n Node Configuration**, 6. **n8n Code JavaScript**, 7. **n8n Code Python**, API / SDK (+19 more)

### Community 23 - "Community 23"
Cohesion: 0.07
Nodes (26): 1. n8n-mcp MCP Server, 2. Claude Access, Advanced Configuration, Custom Skill Location, Installation Guide, Installation Methods, MCP Tools Not Available, Method 1: Claude Code (Recommended) (+18 more)

### Community 24 - "Community 24"
Cohesion: 0.12
Nodes (21): humanizeSource(), DAYS_FULL, MONTHS_SHORT, reportes, buildDayMap(), Cell, DAYS_FULL, fmtDate() (+13 more)

### Community 25 - "Community 25"
Cohesion: 0.07
Nodes (26): 1. Validation is Iterative, 2. False Positives Exist, 3. Auto-Sanitization Works, 4. Profile Matters, 5. Error Messages Help, Author, Common Error Types, Complementary: (+18 more)

### Community 26 - "Community 26"
Cohesion: 0.08
Nodes (25): **10\. Alcance del Proyecto (Scope)**, **1\. Rol del Asistente**, **2\. Descripción General y Visión**, **3\. Alcance Funcional General**, **4\. Reglas de Negocio (Core)**, **5\. Roles y Permisos**, **6\. Solicitudes de Corrección**, **7\. Informes** (+17 more)

### Community 27 - "Community 27"
Cohesion: 0.08
Nodes (25): Additional Parameters, Check Version Compatibility, Common Workflow: Finding & Configuring, Detail Levels (mode="info"), Examples, Explore AI Nodes, Find and Configure HTTP Request, Full Detail (USE SPARINGLY) (+17 more)

### Community 28 - "Community 28"
Cohesion: 0.08
Nodes (23): dependencies, dotenv, exceljs, hono, @hono/node-server, @hono/zod-validator, @microsoft/signalr, pdfkit (+15 more)

### Community 29 - "Community 29"
Cohesion: 0.08
Nodes (23): ADMINS Y MANAGERS, Ajustes, API Keys, Asistencia (Fichajes), Calendario Laboral, Control Horario, Correcciones (Incidencias Admin), Dashboard Admin (+15 more)

### Community 30 - "Community 30"
Cohesion: 0.09
Nodes (23): serviceAccountKeyPath, track, build, development, preview, production, cli, version (+15 more)

### Community 31 - "Community 31"
Cohesion: 0.08
Nodes (23): Auto-Sanitization System, Best Practices, Binary vs Unary Operators, Common Errors, Configuration Validation Tools Guide, Do, Don't, Error Types (+15 more)

### Community 32 - "Community 32"
Cohesion: 0.08
Nodes (23): Activates On, Coverage, Critical Insights, Cross-Cutting Concerns, Dependencies, Deployment Phase, Evaluations, File Count (+15 more)

### Community 33 - "Community 33"
Cohesion: 0.13
Nodes (9): AdminCorrectionsPage(), STATUS_BADGE, TYPE_LABELS, SUGGESTIONS, CorrectionsContext, CorrectionsProvider(), useCorrections(), MobileNav() (+1 more)

### Community 34 - "Community 34"
Cohesion: 0.09
Nodes (22): access_logs, Admin/Manager (/api/users/*, /api/incidencias/*), Antigravity Backend — Guía Completa de Implementación, Empleado (/api/me/*), Endpoints del Backend, Estado de Fases del Proyecto, Humanización de source (para reportes y exports), incidencias (+14 more)

### Community 35 - "Community 35"
Cohesion: 0.10
Nodes (23): dispatchN8nWebhook(), N8nWorkflow, triggerWorkflow(), WEBHOOK_PATHS, assignCardSchema, assignPinSchema, createUserSchema, patchUserSchema (+15 more)

### Community 36 - "Community 36"
Cohesion: 0.10
Nodes (20): compilerOptions, declaration, declarationMap, esModuleInterop, exactOptionalPropertyTypes, forceConsistentCasingInFileNames, lib, module (+12 more)

### Community 37 - "Community 37"
Cohesion: 0.10
Nodes (20): 10. Observabilidad y soporte, 1. Facturación y suscripciones (no existe nada), 2. Onboarding self-service (hoy es manual vía superadmin), 3. Gating por plan y cuotas (no existe), 4. Aislamiento de datos (defensa en profundidad), 5. Infraestructura y escalado (hoy: 1 Raspberry Pi = SPOF), 6. Consola de operador (superadmin) ampliada, 7. Branding y dominios por tenant (+12 more)

### Community 38 - "Community 38"
Cohesion: 0.10
Nodes (19): 1. Expression Syntax (Skill #1), 2. MCP Tools Expert (Skill #2), 3. Workflow Patterns (Skill #3), 4. Validation Expert (Skill #4), 5. Node Configuration (Skill #5), Critical Findings for Skills, Database Statistics, Generated: 2025-10-20 (+11 more)

### Community 40 - "Community 40"
Cohesion: 0.12
Nodes (6): download(), getToken(), supabase, AuthContext, api, request()

### Community 41 - "Community 41"
Cohesion: 0.11
Nodes (17): AcClient, AcCompany, createAcClient(), ListResponse, admin, EmitFn, clockingModesSchema, createCompanySchema (+9 more)

### Community 42 - "Community 42"
Cohesion: 0.11
Nodes (18): 10. Incorrect Property Path, 11. Using = Prefix Outside JSON, 12. Expressions in Webhook Path, 13. Forgetting .json in $node Reference, 14. String Concatenation Confusion, 15. Empty Expression Brackets, 1. Missing Curly Braces, 2. Webhook Body Access (+10 more)

### Community 43 - "Community 43"
Cohesion: 0.17
Nodes (16): decryptSetting(), encryptSetting(), getKey(), inferDetailType(), attachHandlers(), buildConnection(), connections, ConnectionStatus (+8 more)

### Community 44 - "Community 44"
Cohesion: 0.11
Nodes (17): main, main, connections, Build Prompt, Call OpenAI, Extract Answer, Webhook, main (+9 more)

### Community 45 - "Community 45"
Cohesion: 0.11
Nodes (17): Administrador / Manager, 🏗️ Arquitectura, Backend, Empleado, 🔄 Estado de desarrollo, 📁 Estructura del proyecto, Frontend, ✨ Funcionalidades (+9 more)

### Community 46 - "Community 46"
Cohesion: 0.15
Nodes (12): AdminEmployeeDetailPage(), fmtDate(), fmtDateTime(), getInitials(), ROLE_LABELS, SOURCE_LABELS, STATUS_CLS, STATUS_LABELS (+4 more)

### Community 47 - "Community 47"
Cohesion: 0.18
Nodes (10): getEnv(), getSupabaseAdmin(), getSupabasePublic(), supabaseAdmin, supabasePublic, main(), main(), main() (+2 more)

### Community 48 - "Community 48"
Cohesion: 0.12
Nodes (3): ROLE_LABELS, MONTHS_ES, MONTHS_ES

### Community 49 - "Community 49"
Cohesion: 0.15
Nodes (8): AuthContextValue, AuthState, Profile, getAuthHeaders(), supabase, AuthContext, api, request()

### Community 50 - "Community 50"
Cohesion: 0.13
Nodes (14): Activates On, Common Patterns, Core Topics, Coverage, Dependencies, Error Prevention, Evaluations, File Count (+6 more)

### Community 51 - "Community 51"
Cohesion: 0.13
Nodes (14): Activates On, Core Topics, Coverage, Dependencies, Evaluations, File Count, Files, Key Features (+6 more)

### Community 53 - "Community 53"
Cohesion: 0.14
Nodes (13): baseline_without_skill, expected_quality, likely_response, expected_behavior, id, query, skills, with_skill_expected (+5 more)

### Community 54 - "Community 54"
Cohesion: 0.14
Nodes (13): baseline_without_skill, expected_quality, likely_response, expected_behavior, id, query, skills, with_skill_expected (+5 more)

### Community 55 - "Community 55"
Cohesion: 0.14
Nodes (13): baseline_without_skill, expected_quality, likely_response, expected_behavior, id, query, skills, with_skill_expected (+5 more)

### Community 56 - "Community 56"
Cohesion: 0.14
Nodes (13): Call-graph collapse, Candidate card, Cross-section (good for layered shallowness), Diagram patterns, Hand-built boxes-and-arrows (when Mermaid's layout fights you), Header, HTML Report Format, Mass diagram (good for "interface as wide as implementation") (+5 more)

### Community 57 - "Community 57"
Cohesion: 0.14
Nodes (13): baseline_without_skill, expected_quality, likely_response, expected_behavior, id, query, skills, with_skill_expected (+5 more)

### Community 58 - "Community 58"
Cohesion: 0.14
Nodes (13): baseline_without_skill, expected_quality, likely_response, expected_behavior, id, query, skills, with_skill_expected (+5 more)

### Community 59 - "Community 59"
Cohesion: 0.14
Nodes (13): baseline_without_skill, expected_quality, likely_response, expected_behavior, id, query, skills, with_skill_expected (+5 more)

### Community 60 - "Community 60"
Cohesion: 0.14
Nodes (13): baseline_without_skill, expected_quality, likely_response, expected_behavior, id, query, skills, with_skill_expected (+5 more)

### Community 61 - "Community 61"
Cohesion: 0.14
Nodes (13): baseline_without_skill, expected_quality, likely_response, expected_behavior, id, query, skills, with_skill_expected (+5 more)

### Community 62 - "Community 62"
Cohesion: 0.14
Nodes (13): Credential Management, Data Table Management, Most Used Tools (by success rate), n8n_audit_instance, n8n_manage_credentials, n8n_manage_datatable, n8n MCP Tools Expert, Performance Characteristics (+5 more)

### Community 63 - "Community 63"
Cohesion: 0.14
Nodes (13): connections, Construir prompt, Extraer respuesta, Llamar Anthropic API, Webhook, main, main, main (+5 more)

### Community 64 - "Community 64"
Cohesion: 0.14
Nodes (13): 1. State the question and pick N, 2. Generate radically different variants, 3. Wire them together, 4. Build the floating switcher, 5. Hand it over, 6. Capture the answer and clean up, Anti-patterns, Process (+5 more)

### Community 65 - "Community 65"
Cohesion: 0.14
Nodes (13): 2N Access Commander — API Reference, Comandos SignalR, Data Query — Filtrado y paginación, Dispositivos físicos (HTTP API — diferente al REST de AC), Endpoints principales, Grupos y Zonas, Hub URL v3, REST API v3 (+5 more)

### Community 66 - "Community 66"
Cohesion: 0.21
Nodes (7): AdminSettingsPage(), DEFAULT_CLOCKING_MODES, DEFAULT_SETTINGS, ThemeContext, ThemeProvider(), useTheme(), ProfilePage()

### Community 67 - "Community 67"
Cohesion: 0.17
Nodes (9): FormState, Incidencia, modal, NewIncidenciaModal(), STATUS_STYLE, styles, todayISO(), TYPE_LABEL (+1 more)

### Community 68 - "Community 68"
Cohesion: 0.15
Nodes (12): baseline_without_skill, expected_quality, likely_response, expected_behavior, id, query, skills, with_skill_expected (+4 more)

### Community 69 - "Community 69"
Cohesion: 0.17
Nodes (11): 2N Access Commander, Agent skills, Backend (desde /backend), Domain docs, Frontend, graphify, Issue tracker, n8n (+3 more)

### Community 70 - "Community 70"
Cohesion: 0.17
Nodes (11): Diagnose, Iterate on the loop itself, Non-deterministic bugs, Phase 1 — Build a feedback loop, Phase 2 — Reproduce, Phase 3 — Hypothesise, Phase 4 — Instrument, Phase 5 — Fix + regression test (+3 more)

### Community 71 - "Community 71"
Cohesion: 0.20
Nodes (6): DETAIL_BADGE, fmtMinutes(), HistoryPage(), periodSubtitle(), RANGE_LABELS, SOURCE_LABELS

### Community 72 - "Community 72"
Cohesion: 0.17
Nodes (11): Bloque 1 — Apertura y UI básica, Bloque 2 — Sugerencias rápidas (camino feliz), Bloque 3 — Preguntas libres cubriendo el manual, Bloque 4 — Conciencia del contexto de página (`page` pathname), Bloque 5 — Conversación multi-turno (historial), Bloque 6 — Casos límite y errores, Bloque 7 — Renderizado de markdown, Bloque 8 — Infraestructura y trazabilidad (+3 more)

### Community 73 - "Community 73"
Cohesion: 0.17
Nodes (11): 1. Obtención de Credenciales (API Key), 2. Autenticación y Cabeceras, 3. Especificación de Endpoints, 4. Ejemplos de Implementación en Múltiples Lenguajes, 5. Control de Errores Comunes, === BASH / cURL ===, Ejemplo de Respuesta Correcta (`200 OK`), `GET /api/external/v1/fichajes` (+3 more)

### Community 74 - "Community 74"
Cohesion: 0.17
Nodes (12): 6 Actions, Create Credential, Delete Credential, Discover Schema, Find Which Workflows Use a Credential, Get Credential, List Credentials, n8n_manage_credentials (CREDENTIAL MANAGEMENT) (+4 more)

### Community 75 - "Community 75"
Cohesion: 0.17
Nodes (11): Auto-Fix Capabilities, Best Practices, Detailed Guides, ✅ Do, ❌ Don't, Example, n8n Validation Expert, Pattern from Telemetry (+3 more)

### Community 76 - "Community 76"
Cohesion: 0.17
Nodes (11): 1. State the question, 2. Pick the language, 3. Isolate the logic in a portable module, 4. Build the smallest TUI that exposes the state, 5. Make it runnable in one command, 6. Hand it over, 7. Capture the answer, Anti-patterns (+3 more)

### Community 77 - "Community 77"
Cohesion: 0.17
Nodes (11): 1. Gather context, 2. Explore the codebase (optional), 3. Draft vertical slices, 4. Quiz the user, 5. Publish the issues to the issue tracker, Acceptance criteria, Blocked by, Parent (+3 more)

### Community 78 - "Community 78"
Cohesion: 0.17
Nodes (11): Bad agent brief, Behavioral, not procedural, Complete acceptance criteria, Durability over precision, Examples, Explicit scope boundaries, Good agent brief (bug), Good agent brief (enhancement) (+3 more)

### Community 79 - "Community 79"
Cohesion: 0.18
Nodes (10): author, name, url, description, homepage, keywords, license, name (+2 more)

### Community 80 - "Community 80"
Cohesion: 0.18
Nodes (11): 2. Top 10 Successful CODE Node Patterns, Pattern 10: String Aggregation & Reporting, Pattern 1: Multi-source Data Aggregation, Pattern 2: Regex Filtering & Pattern Matching, Pattern 3: Markdown Parsing & Structured Data Extraction, Pattern 4: JSON Comparison & Validation, Pattern 5: CRM Data Transformation, Pattern 6: Release Information Processing (+3 more)

### Community 81 - "Community 81"
Cohesion: 0.24
Nodes (9): calcWorkingDays(), HOLIDAY_TYPE_LABELS, MonthGrid(), MONTHS_ES, STATUS_CLS, STATUS_LABELS, toDateStr(), TYPE_CONFIG (+1 more)

### Community 82 - "Community 82"
Cohesion: 0.18
Nodes (10): Critical: Webhook Data Structure, Essential Rules, ⚠️ Important: JavaScript First, Integration with Other Skills, Most Useful Modules, Python Code Node (Beta), Quick Reference Checklist, Quick Start (+2 more)

### Community 83 - "Community 83"
Cohesion: 0.18
Nodes (11): Create Channel, Empty Check (Unary), GET Request, HTTP Request Node Examples, IF Node Examples, Operation-Specific Configuration, Post Message, POST with JSON (+3 more)

### Community 84 - "Community 84"
Cohesion: 0.18
Nodes (11): 1. ❌ Wrong: Connecting tools to main port, 2. ❌ Wrong: Vague tool descriptions, 3. ❌ Wrong: No memory for conversations, 4. ❌ Wrong: Giving AI write access, 5. ❌ Wrong: Unbounded tool responses, Common Gotchas, ✅ Correct: Add memory, ✅ Correct: Limit tool output (+3 more)

### Community 85 - "Community 85"
Cohesion: 0.18
Nodes (11): 1. ❌ Wrong: Ignoring timezone, 2. ❌ Wrong: Overlapping executions, 3. ❌ Wrong: No error handling, 4. ❌ Wrong: Processing all data at once, 5. ❌ Wrong: Hardcoded dates, Common Gotchas, ✅ Correct: Add error workflow, ✅ Correct: Add execution lock (+3 more)

### Community 86 - "Community 86"
Cohesion: 0.18
Nodes (10): n8n Workflows — Pasos Exactos, WF credential-card, WF credential-pin, WF incidencia-nueva, WF incidencia-resuelta, WF reconciliation (Cron 2:00 AM), WF sync-retry (Cron cada 15 min), WF user-create (+2 more)

### Community 87 - "Community 87"
Cohesion: 0.35
Nodes (10): __dirname, envLines, envPath, err(), hdr(), httpRequest(), main(), ok() (+2 more)

### Community 88 - "Community 88"
Cohesion: 0.20
Nodes (6): DayGroup, DETAIL_LABEL, Fichaje, HistorialResponse, SOURCE_LABEL, styles

### Community 89 - "Community 89"
Cohesion: 0.20
Nodes (9): correct_code_pattern, description, expected_behavior, expected_output_includes, id, name, query, should_not_include (+1 more)

### Community 90 - "Community 90"
Cohesion: 0.20
Nodes (9): correct_code_pattern, description, expected_behavior, expected_output_includes, id, name, query, should_emphasize (+1 more)

### Community 91 - "Community 91"
Cohesion: 0.20
Nodes (9): correct_code_pattern, description, expected_behavior, expected_output_includes, id, name, query, should_not_include (+1 more)

### Community 92 - "Community 92"
Cohesion: 0.20
Nodes (9): correct_modules, description, expected_behavior, expected_output_includes, id, name, query, should_not_include (+1 more)

### Community 93 - "Community 93"
Cohesion: 0.20
Nodes (9): 3. Common Data Access Patterns, 3. Return Value Structures, 8. Quick Reference Checklist, Executive Summary, Key Recommendations, n8n CODE Node - Best Practices & Pattern Analysis, Recommended Return Patterns, Usage Distribution (+1 more)

### Community 94 - "Community 94"
Cohesion: 0.20
Nodes (10): 6. Library & Built-in Availability, Common Code Patterns, Core Data Access, Data Querying with JMESPath, Date & Time Helpers (Luxon-based), n8n Built-in Methods & Variables (JavaScript), Node & Workflow Context, Python Built-in Methods (Beta) (+2 more)

### Community 95 - "Community 95"
Cohesion: 0.20
Nodes (9): Challenge against the glossary, Cross-reference with code, Discuss concrete scenarios, Domain awareness, During the session, File structure, Offer ADRs sparingly, Sharpen fuzzy language (+1 more)

### Community 96 - "Community 96"
Cohesion: 0.20
Nodes (9): n8n_create_workflow, n8n_deploy_template (QUICK START!), n8n_get_workflow, n8n_test_workflow (TRIGGER EXECUTION), n8n_validate_workflow (by ID), Summary, Tool Availability, Workflow Lifecycle (+1 more)

### Community 97 - "Community 97"
Cohesion: 0.20
Nodes (10): 19 Operation Types, Activation Operations, AI Connection Types (8 types), Cleanup & Recovery, Example Usage, Intent Parameter (IMPORTANT!), n8n_update_partial_workflow (MOST USED!), patchNodeField (Surgical String Edits) (+2 more)

### Community 98 - "Community 98"
Cohesion: 0.20
Nodes (10): Basic Webhook, DELETE Request, GET Request, HTTP & API Nodes, HTTP Request (nodes-base.httpRequest), POST with JSON, PUT/PATCH Request, Webhook (nodes-base.webhook) (+2 more)

### Community 99 - "Community 99"
Cohesion: 0.20
Nodes (10): 1. Binary Operators (Two Values), 1. Broken Connections, 2. Branch Count Mismatches, 2. Unary Operators (One Value), 3. IF/Switch Metadata, 3. Paradoxical Corrupt States, Auto-Sanitization System, What It CANNOT Fix (+2 more)

### Community 100 - "Community 100"
Cohesion: 0.20
Nodes (9): Detailed Pattern Files, Integration with Other Skills, n8n Workflow Patterns, Pattern Selection Guide, Pattern Statistics, Real Template Examples, Summary, The 6 Core Patterns (+1 more)

### Community 101 - "Community 101"
Cohesion: 0.20
Nodes (9): 1. Planning, 2. Tracer Bullet, 3. Incremental Loop, 4. Refactor, Anti-Pattern: Horizontal Slices, Checklist Per Cycle, Philosophy, Test-Driven Development (+1 more)

### Community 102 - "Community 102"
Cohesion: 0.20
Nodes (9): Invocation, Needs-info template, Quick state override, Reference docs, Resuming a previous session, Roles, Show what needs attention, Triage (+1 more)

### Community 103 - "Community 103"
Cohesion: 0.22
Nodes (8): description, expected_behavior, expected_output_includes, id, name, query, should_not_include, skill

### Community 104 - "Community 104"
Cohesion: 0.22
Nodes (8): 1. In-process, 2. Local-substitutable, 3. Remote but owned (Ports & Adapters), 4. True external (Mock), Deepening, Dependency categories, Seam discipline, Testing strategy: replace, don't layer

### Community 105 - "Community 105"
Cohesion: 0.22
Nodes (8): Critical: Webhook Data Structure, Essential Rules, Integration with Other Skills, JavaScript Code Node, Quick Reference Checklist, Quick Start, When to Use Code Node, Works With:

### Community 106 - "Community 106"
Cohesion: 0.22
Nodes (9): Common Mistakes, Mistake 1: Wrong nodeType Format, Mistake 2: Using detail="full" by Default, Mistake 3: Not Using Validation Profiles, Mistake 4: Ignoring Auto-Sanitization, Mistake 5: Not Using Smart Parameters, Mistake 6: Not Using intent Parameter, Mistake 7: Wrong Parameter Name for updateNode (+1 more)

### Community 107 - "Community 107"
Cohesion: 0.22
Nodes (9): Basic Switch, Boolean Comparison, Conditional Nodes, IF (nodes-base.if), Multiple Conditions (AND), Multiple Conditions (OR), Number Comparison, String Comparison (+1 more)

### Community 108 - "Community 108"
Cohesion: 0.22
Nodes (8): Configuration Philosophy, Detailed References, Google Sheets Node, n8n Node Configuration, Node-Specific Configuration Notes, SplitInBatches v3, Summary, Surgical Field Edits with patchNodeField

### Community 109 - "Community 109"
Cohesion: 0.22
Nodes (8): Batch Processing, Database Operations Pattern, Pattern 1: Split In Batches, Pattern 2: Paginated Queries, Pattern 3: Cursor-Based Pagination, Pattern Structure, Real Template Examples, Summary

### Community 110 - "Community 110"
Cohesion: 0.22
Nodes (9): 1. ❌ Wrong: Unbounded queries, 2. ❌ Wrong: String concatenation in queries, 3. ❌ Wrong: No transaction for multi-step operations, 4. ❌ Wrong: Processing all items at once, Common Gotchas, ✅ Correct: Batch processing, ✅ Correct: Parameterized queries, ✅ Correct: Use LIMIT (+1 more)

### Community 111 - "Community 111"
Cohesion: 0.22
Nodes (8): Combine Multiple API Responses, Extract Nested Data, Flatten Arrays, HTTP API Integration Pattern, Pattern Structure, Real Template Examples, Response Transformation, Summary

### Community 112 - "Community 112"
Cohesion: 0.22
Nodes (9): 1. ❌ Wrong: Hardcoded URLs, 2. ❌ Wrong: Credentials in parameters, 3. ❌ Wrong: No error handling, 4. ❌ Wrong: Blocking on large responses, Common Gotchas, ✅ Correct: Handle errors, ✅ Correct: Use batching, ✅ Correct: Use credentials system (+1 more)

### Community 113 - "Community 113"
Cohesion: 0.22
Nodes (9): 1. ❌ Wrong: Accessing webhook data, 2. ❌ Wrong: Response mode confusion, 3. ❌ Wrong: No validation, 4. ❌ Wrong: Hardcoded paths, Common Gotchas, ✅ Correct, ✅ Correct, ✅ Correct (+1 more)

### Community 114 - "Community 114"
Cohesion: 0.22
Nodes (8): Further Notes, Implementation Decisions, Out of Scope, Problem Statement, Process, Solution, Testing Decisions, User Stories

### Community 115 - "Community 115"
Cohesion: 0.22
Nodes (8): Directory structure, File format, Naming the file, Out-of-Scope Knowledge Base, Updating or removing out-of-scope files, When to check `.out-of-scope/`, When to write to `.out-of-scope/`, Writing the reason

### Community 116 - "Community 116"
Cohesion: 0.22
Nodes (8): Description Requirements, Process, Review Checklist, SKILL.md Template, Skill Structure, When to Add Scripts, When to Split Files, Writing Skills

### Community 117 - "Community 117"
Cohesion: 0.32
Nodes (7): DashboardData, DashboardScreen(), fmtMinutes(), fmtTime(), STATUS_LABEL, styles, WeekBar

### Community 118 - "Community 118"
Cohesion: 0.25
Nodes (7): expected_behavior, expected_content, id, notes, priority, query, skills

### Community 119 - "Community 119"
Cohesion: 0.25
Nodes (7): expected_behavior, expected_content, id, notes, priority, query, skills

### Community 120 - "Community 120"
Cohesion: 0.25
Nodes (7): expected_behavior, expected_content, id, notes, priority, query, skills

### Community 121 - "Community 121"
Cohesion: 0.25
Nodes (7): expected_behavior, expected_content, id, notes, priority, query, skills

### Community 122 - "Community 122"
Cohesion: 0.25
Nodes (7): expected_behavior, expected_content, id, notes, priority, query, skills

### Community 123 - "Community 123"
Cohesion: 0.25
Nodes (8): 2. Python Code Examples & Best Practices, Python Best Practices, Python Pattern 1: Data Transformation (Run Once for All Items), Python Pattern 2: Filtering & Aggregation, Python Pattern 3: String Processing with Regex, Python Pattern 4: Data Science Operations, Python Pattern 5: Dictionary/Object Manipulation, Python vs JavaScript: Key Differences

### Community 124 - "Community 124"
Cohesion: 0.25
Nodes (8): Creating a New Skill, Step 1: Define Scope, Step 2: Create Evaluations, Step 3: Test MCP Tools, Step 4: Write SKILL.md, Step 5: Add Reference Files, Step 6: Test Against Evaluations, Step 7: Document Metadata

### Community 125 - "Community 125"
Cohesion: 0.25
Nodes (8): Basic Usage, Example 1: Filter Active Items, Example 2: Transform All Items, Example 3: Aggregate Data, Example 4: Sort and Limit, Example 5: Group By Category, Example 6: Deduplicate by ID, Pattern 1: $input.all() - Process All Items

### Community 126 - "Community 126"
Cohesion: 0.25
Nodes (7): Aggregate from Items, Basic Statistics, Complete Standard Library List, ⚠️ Critical Limitation, Module 9: statistics - Statistical Functions, Standard Library Reference - Python Code Node, Summary

### Community 127 - "Community 127"
Cohesion: 0.25
Nodes (8): Common Pitfalls, How It Works, n8n_generate_workflow (NATURAL LANGUAGE → WORKFLOW), Parameters, Path A: Proposals → Deploy (default, recommended), Path B: Skip Cache → Preview → Confirm, Recommended Follow-Up, Writing a Good Description

### Community 128 - "Community 128"
Cohesion: 0.25
Nodes (8): Communication Nodes, Create Channel, Get Email, Gmail (nodes-base.gmail), Post Message, Send Email, Slack (nodes-base.slack), Update Message

### Community 129 - "Community 129"
Cohesion: 0.25
Nodes (8): 1. "Missing error handling", 2. "No retry logic", 3. "Missing rate limiting", 4. "Unbounded query", Common False Positives, False Positives, Reducing False Positives, What Are They?

### Community 130 - "Community 130"
Cohesion: 0.25
Nodes (8): Bulk INSERT, Database Node Configuration, Find Documents, Insert Document, MongoDB, MySQL, SELECT with JOIN, Update Document

### Community 131 - "Community 131"
Cohesion: 0.25
Nodes (8): 1. Daily Reports, 2. Data Synchronization, 3. Monitoring & Health Checks, 4. Cleanup & Maintenance, 5. Data Enrichment, 6. Backup Automation, 7. Content Publishing, Common Use Cases

### Community 132 - "Community 132"
Cohesion: 0.25
Nodes (7): lastNode (Custom Response), onReceived (Default), Pattern Structure, Real Template Examples, Response Modes, Summary, Webhook Processing Pattern

### Community 133 - "Community 133"
Cohesion: 0.25
Nodes (7): FASE 0 — Setup, FASE 1 — Auth + Perfil, FASE 2 — Fichajes (Tres Modos), FASE 3 — Incidencias, FASE 4 — Reportes, FASE 5 — Sync 2N AC, Prompts por Fase para Claude Code

### Community 134 - "Community 134"
Cohesion: 0.25
Nodes (7): Algoritmo de emparejamiento in/out, Los 4 valores de source, Modo 2N — SignalR Listener, Modo Corrección — al aprobar incidencia tipo 'olvido', Modo Web y Móvil — POST /api/me/fichar, Modos de Fichaje — Implementación Detallada, Tabla de comportamiento por modo

### Community 135 - "Community 135"
Cohesion: 0.29
Nodes (7): adminData, adminRoutes, employeeData, employeeRoutes, outDir, run(), takeScreenshot()

### Community 136 - "Community 136"
Cohesion: 0.25
Nodes (7): 1. Explore, 2. Present findings and ask, 3. Confirm and edit, 4. Write, 5. Done, Process, Setup Matt Pocock's Skills

### Community 137 - "Community 137"
Cohesion: 0.29
Nodes (6): 2N Access Commander, Backend (desde /backend), Frontend, n8n, Otros, Supabase

### Community 138 - "Community 138"
Cohesion: 0.29
Nodes (3): ProfileData, ROLE_LABEL, styles

### Community 139 - "Community 139"
Cohesion: 0.29
Nodes (6): description, name, owner, name, url, plugins

### Community 140 - "Community 140"
Cohesion: 0.29
Nodes (6): Changelog, Development Guide, License, Release Process, Repository Structure, Version Numbering

### Community 141 - "Community 141"
Cohesion: 0.29
Nodes (6): ADR Format, Numbering, Optional sections, Template, What qualifies, When to offer an ADR

### Community 142 - "Community 142"
Cohesion: 0.29
Nodes (6): 1. Explore, 2. Present candidates as an HTML report, 3. Grilling loop, Glossary, Improve Codebase Architecture, Process

### Community 143 - "Community 143"
Cohesion: 0.29
Nodes (6): compilerOptions, paths, strict, extends, include, @/*

### Community 144 - "Community 144"
Cohesion: 0.29
Nodes (6): Choosing the Right Pattern, Data Access Patterns - JavaScript Code Node, Decision Tree, Overview, Quick Reference Table, Summary

### Community 145 - "Community 145"
Cohesion: 0.29
Nodes (7): Basic Usage, Example 1: Process Single API Response, Example 2: Transform Single Object, Example 3: Validate Single Item, Example 4: Extract Nested Data, Example 5: Combine with Other Methods, Pattern 2: $input.first() - Get First Item

### Community 146 - "Community 146"
Cohesion: 0.29
Nodes (7): Common Webhook Scenarios, Critical: Webhook Data Structure, Example: Full Webhook Processing, POST Data, Query Params, and Headers, Structure, The Problem, Wrong vs Right

### Community 147 - "Community 147"
Cohesion: 0.29
Nodes (7): Array Safety, Error #5: Missing Null Checks / Undefined Access, Object Property Safety, Safe Access Patterns, The Problem, The Solution, Webhook Data Safety

### Community 148 - "Community 148"
Cohesion: 0.29
Nodes (6): 1. Always Validate Input Data, 2. Use Try-Catch for Error Handling, 3. Prefer Array Methods Over Loops, 4. Filter Early, Process Late, 5. Use Descriptive Variable Names, Best Practices

### Community 149 - "Community 149"
Cohesion: 0.29
Nodes (6): AI Nodes, Chat Completion, OpenAI (nodes-langchain.openAi), Operation Patterns Guide, Overview, Summary

### Community 150 - "Community 150"
Cohesion: 0.29
Nodes (7): Common Dependency Patterns, displayOptions Mechanism, Finding Property Dependencies, Pattern 1: Boolean Toggle, Pattern 2: Operation Switch, Pattern 3: Type Selection, Property Dependencies Deep Dive

### Community 151 - "Community 151"
Cohesion: 0.29
Nodes (7): 1. Broken Connections, 2. Circular Dependencies, 3. Multiple Start Nodes, 4. Disconnected Nodes, Common Workflow Errors, validate_workflow (Structure), Workflow Validation

### Community 152 - "Community 152"
Cohesion: 0.29
Nodes (7): 1. Check `valid` field, 2. Fix errors first, 3. Review warnings, 4. Consider suggestions, Complete Response, How to Read It, Validation Result Structure

### Community 153 - "Community 153"
Cohesion: 0.29
Nodes (7): 1. missing_required, 2. invalid_value, 3. type_mismatch, 4. invalid_expression, 5. invalid_reference, 6. patchNodeField Errors, Common Error Types

### Community 154 - "Community 154"
Cohesion: 0.29
Nodes (6): AI Agent Workflow Pattern, Core AI Connection Types, Pattern Structure, Real Template Examples, Security: Treat Tool Output as Untrusted Input, Summary

### Community 155 - "Community 155"
Cohesion: 0.29
Nodes (7): 1. Trigger, 2. AI Agent Node, 3. Language Model, 4. Tools (ANY Node Can Be a Tool!), 5. Memory (Optional but Recommended), 6. Output Processing, Core Components

### Community 156 - "Community 156"
Cohesion: 0.29
Nodes (7): AI Agent Tool (Sub-Agents), Code Node as Tool, Database as Tool, Making ANY Node an AI Tool, MCP Client Tool, Pre-built Tool Nodes, Tool Configuration

### Community 157 - "Community 157"
Cohesion: 0.29
Nodes (7): Error Responses (400-599), Handling API Responses, Pagination, Pattern 1: Offset-based, Pattern 2: Cursor-based, Pattern 3: Link Header, Success Response (200-299)

### Community 158 - "Community 158"
Cohesion: 0.29
Nodes (7): Checklist for Scheduled Workflows, Deployment, Error Handling, Implementation, Monitoring, Planning, Testing

### Community 159 - "Community 159"
Cohesion: 0.33
Nodes (5): Before exploring, read these, Domain Docs, File structure, Flag ADR conflicts, Use the glossary's vocabulary

### Community 160 - "Community 160"
Cohesion: 0.33
Nodes (6): 4. Top 5 Error Patterns to Avoid, Error #1: Empty Code (23% of validation failures), Error #2: Missing Return Statement (15% of validation failures), Error #3: Expression Syntax Confusion (8% of validation failures), Error #4: Unmatched Expression Brackets (6% of validation failures), Error #5: Incorrect Return Wrapper (5% of validation failures)

### Community 161 - "Community 161"
Cohesion: 0.33
Nodes (6): 9. Additional Resources, Common Use Cases Quick Reference, n8n Community Resources, Official n8n Documentation, Related n8n Nodes, When to Use CODE Node vs Other Nodes

### Community 163 - "Community 163"
Cohesion: 0.33
Nodes (5): 1. Frame the problem space, 2. Spawn sub-agents, 3. Present and compare, Interface Design, Process

### Community 164 - "Community 164"
Cohesion: 0.33
Nodes (5): Language, Principles, Rejected framings, Relationships, Terms

### Community 165 - "Community 165"
Cohesion: 0.33
Nodes (6): Basic Usage, Example 1: Add Processing Metadata, Example 2: Per-Item Validation, Example 3: Item-Specific API Call, Example 4: Conditional Processing, Pattern 3: $input.item - Current Item (Each Item Mode)

### Community 166 - "Community 166"
Cohesion: 0.33
Nodes (6): Common Mistakes, Mistake 1: Using $json Without Context, Mistake 2: Forgetting .json Property, Mistake 3: Using $input.item in All Items Mode, Mistake 4: Not Handling Empty Arrays, Mistake 5: Modifying Original Data

### Community 167 - "Community 167"
Cohesion: 0.33
Nodes (6): Code Structure, Data Safety, Error Prevention Checklist, Return Format, Syntax, Testing

### Community 168 - "Community 168"
Cohesion: 0.33
Nodes (6): #1: Empty Code or Missing Return (Most Common), #2: Expression Syntax Confusion, #3: Incorrect Return Wrapper, #4: Missing Null Checks, #5: Webhook Body Nesting, Error Prevention - Top 5 Mistakes

### Community 169 - "Community 169"
Cohesion: 0.33
Nodes (6): 1. Multi-Source Data Aggregation, 2. Filtering with Regex, 3. Data Transformation & Enrichment, 4. Top N Filtering & Ranking, 5. Aggregation & Reporting, Common Patterns Overview

### Community 170 - "Community 170"
Cohesion: 0.33
Nodes (6): Correct Node Reference Syntax, Cross-Iteration Data Accumulation (CRITICAL), Float Precision for Price/Currency Comparison, pairedItem for New Output Items, Production Gotchas, SplitInBatches Loop Semantics

### Community 171 - "Community 171"
Cohesion: 0.33
Nodes (6): 1. Always Use .get() for Dictionary Access, 2. Handle None/Null Values Explicitly, 3. Use List Comprehensions for Filtering, 4. Return Consistent Structure, 5. Debug with print() Statements, Best Practices

### Community 172 - "Community 172"
Cohesion: 0.33
Nodes (6): 1. Data Transformation, 2. Filtering & Aggregation, 3. String Processing with Regex, 4. Data Validation, 5. Statistical Analysis, Common Patterns Overview

### Community 173 - "Community 173"
Cohesion: 0.33
Nodes (6): #1: Importing External Libraries (Python-Specific!), #2: Empty Code or Missing Return, #3: Incorrect Return Format, #4: KeyError on Dictionary Access, #5: Webhook Body Nesting, Error Prevention - Top 5 Mistakes

### Community 174 - "Community 174"
Cohesion: 0.33
Nodes (6): Compare Dates, Current Date and Time, Date Calculations, Format Dates, Module 2: datetime - Date and Time, Parse Date String

### Community 175 - "Community 175"
Cohesion: 0.33
Nodes (6): Extract All Matches, Module 3: re - Regular Expressions, Pattern Matching, Replace Patterns, Split on Pattern, Validate Format

### Community 176 - "Community 176"
Cohesion: 0.33
Nodes (6): Delete Versions, Get Specific Version, List Versions, n8n_workflow_versions (VERSION CONTROL), Prune Old Versions, Rollback to Previous Version

### Community 177 - "Community 177"
Cohesion: 0.33
Nodes (6): Examples, n8n_audit_instance (SECURITY AUDIT), Output, Remediation Types, Secret Masking, Two Scanning Approaches

### Community 178 - "Community 178"
Cohesion: 0.33
Nodes (6): Code (nodes-base.code), Data Transformation Nodes, Set Fixed Values, Set from Input Data, Set (nodes-base.set), Simple Transformation

### Community 179 - "Community 179"
Cohesion: 0.33
Nodes (6): 1. Choose Right Model, 2. Limit Context Window, 3. Optimize Tool Descriptions, 4. Cache Embeddings, 5. Async Tools for Slow Operations, Performance Optimization

### Community 180 - "Community 180"
Cohesion: 0.33
Nodes (6): 1. Conversational Chatbot, 2. Document Q&A, 3. Data Analysis Assistant, 4. Workflow Automation Agent, 5. Email Processing Agent, Common Use Cases

### Community 181 - "Community 181"
Cohesion: 0.33
Nodes (6): 1. Read-Only Database Tools, 2. Validate Tool Inputs, 3. Rate Limiting, 4. Sanitize User Input, 5. Monitor Tool Usage, Security Considerations

### Community 182 - "Community 182"
Cohesion: 0.33
Nodes (6): 1. Start with Manual Trigger, 2. Test Tools Independently, 3. Test with Standard Questions, 4. Monitor Token Usage, 5. Test Edge Cases, Testing AI Agents

### Community 183 - "Community 183"
Cohesion: 0.33
Nodes (6): Checklist for AI Agent Workflows, Deployment, Implementation, Planning, Security, Testing

### Community 184 - "Community 184"
Cohesion: 0.33
Nodes (6): 1. Data Synchronization, 2. ETL (Extract, Transform, Load), 3. Data Validation & Cleanup, 4. Backup & Archive, 5. Real-time Data Updates, Common Use Cases

### Community 185 - "Community 185"
Cohesion: 0.33
Nodes (6): 1. Trigger, 2. Database Read Nodes, 3. Transform, 4. Database Write Nodes, 5. Verification, Core Components

### Community 186 - "Community 186"
Cohesion: 0.33
Nodes (6): 1. Use Indexes, 2. Limit Result Sets, 3. Use Prepared Statements, 4. Batch Writes, 5. Connection Pooling, Performance Optimization

### Community 187 - "Community 187"
Cohesion: 0.33
Nodes (6): Checklist for Database Workflows, Implementation, Performance, Planning, Reliability, Security

### Community 188 - "Community 188"
Cohesion: 0.33
Nodes (6): 1. Data Fetching & Storage, 2. API to API Integration, 3. Data Enrichment, 4. Monitoring & Alerting, 5. Batch Processing, Common Use Cases

### Community 189 - "Community 189"
Cohesion: 0.33
Nodes (6): 1. None (Public APIs), 2. Bearer Token (Most Common), 3. API Key (Header or Query), 4. Basic Auth, 5. OAuth2, Authentication Methods

### Community 190 - "Community 190"
Cohesion: 0.33
Nodes (6): 1. Test with Manual Trigger, 2. Use Postman/Insomnia First, 3. Log Responses, 4. Check Execution Data, 5. Use Binary Data Properly, Testing & Debugging

### Community 191 - "Community 191"
Cohesion: 0.33
Nodes (6): 1. Trigger, 2. HTTP Request Node, 3. Response Processing, 4. Action, 5. Error Handler, Core Components

### Community 192 - "Community 192"
Cohesion: 0.33
Nodes (6): Checklist for API Integration, Error Handling, Implementation, Performance, Planning, Security

### Community 193 - "Community 193"
Cohesion: 0.33
Nodes (6): DELETE Request, GET Request, POST Request (Form Data), POST Request (JSON Body), PUT/PATCH Request (Update), Request Configuration

### Community 194 - "Community 194"
Cohesion: 0.33
Nodes (6): 1. Schedule Trigger, 2. Data Source, 3. Processing, 4. Delivery, 5. Logging, Core Components

### Community 195 - "Community 195"
Cohesion: 0.33
Nodes (6): 1. Triggers, 2. Data Sources, 3. Transformation, 4. Outputs, 5. Error Handling, Common Workflow Components

### Community 196 - "Community 196"
Cohesion: 0.33
Nodes (6): 1. Webhook Data Structure, 2. Multiple Input Items, 3. Authentication Issues, 4. Node Execution Order, 5. Expression Errors, Common Gotchas

### Community 197 - "Community 197"
Cohesion: 0.33
Nodes (6): API Pagination, Batch Processing Pattern, Cross-Iteration Data, Dry-Run / Verification Tolerance, Nested Loops, SplitInBatches Loop

### Community 198 - "Community 198"
Cohesion: 0.33
Nodes (6): Branching Flow, Data Flow Patterns, Error Handler Pattern, Linear Flow, Loop Pattern, Parallel Processing

### Community 199 - "Community 199"
Cohesion: 0.33
Nodes (6): Example 1: Simple Webhook → Slack, Example 2: Scheduled Report, Example 3: Database Sync, Example 4: AI Assistant, Example 5: API Integration, Quick Start Examples

### Community 200 - "Community 200"
Cohesion: 0.33
Nodes (6): 1. Form Submissions, 2. Payment Webhooks (Stripe, PayPal), 3. Chat Platform Integrations (Slack, Discord, Teams), 4. GitHub/GitLab Webhooks, 5. IoT Device Data, Common Use Cases

### Community 201 - "Community 201"
Cohesion: 0.33
Nodes (6): 1. Webhook Node (Trigger), 2. Validation (Optional but Recommended), 3. Transformation, 4. Action, 5. Response (If responseMode: "lastNode"), Core Components

### Community 202 - "Community 202"
Cohesion: 0.33
Nodes (6): Checklist for Webhook Workflows, Data Handling, Error Handling, Security, Setup, Testing

### Community 203 - "Community 203"
Cohesion: 0.33
Nodes (5): Before exploring, read these, Domain Docs, File structure, Flag ADR conflicts, Use the glossary's vocabulary

### Community 204 - "Community 204"
Cohesion: 0.15
Nodes (3): defaultClockingModes, defaultForm, TABS

### Community 205 - "Community 205"
Cohesion: 0.33
Nodes (5): buildCommand, framework, headers, outputDirectory, rewrites

### Community 206 - "Community 206"
Cohesion: 0.40
Nodes (4): Conventions, Issue tracker: GitHub, When a skill says "fetch the relevant ticket", When a skill says "publish to the issue tracker"

### Community 207 - "Community 207"
Cohesion: 0.40
Nodes (4): Auto-Clarity Exception, Examples, Persistence, Rules

### Community 208 - "Community 208"
Cohesion: 0.40
Nodes (5): 10. Summary & Key Takeaways, Common Pitfalls to Avoid, Essential Rules for CODE Node Success, Next Steps, Success Metrics from Real-World Data

### Community 209 - "Community 209"
Cohesion: 0.40
Nodes (5): 1. Node Configuration Essentials, Choosing the Right Mode, Language Selection, **Run Once for All Items** (Default - Recommended for most use cases), **Run Once for Each Item**

### Community 210 - "Community 210"
Cohesion: 0.40
Nodes (5): 1. Evaluation-Driven Development (EDD), 2. MCP-Informed Content, 3. Keep Skills Concise, 4. Real Examples Only, Development Philosophy

### Community 211 - "Community 211"
Cohesion: 0.40
Nodes (5): Code Standards, Content Quality, Documentation, Quality Checklist, Testing

### Community 212 - "Community 212"
Cohesion: 0.50
Nodes (3): DashboardPage(), fmtMinutes(), pauseOptions

### Community 213 - "Community 213"
Cohesion: 0.40
Nodes (4): CONTEXT.md Format, Rules, Single vs multi-context repos, Structure

### Community 214 - "Community 214"
Cohesion: 0.40
Nodes (5): Basic Usage, Example 1: Combine Multiple Sources, Example 2: Compare Across Nodes, Example 3: Access Node Metadata, Pattern 4: $node - Reference Other Nodes

### Community 215 - "Community 215"
Cohesion: 0.40
Nodes (4): Error Patterns - JavaScript Code Node, Overview, Quick Error Reference, Summary

### Community 216 - "Community 216"
Cohesion: 0.40
Nodes (4): 2. Return Intermediate Results, 3. Try-Catch for Troubleshooting, 4. Validate Input Structure, Debugging Tips

### Community 217 - "Community 217"
Cohesion: 0.40
Nodes (5): Best Practices, Error #4: Unmatched Expression Brackets, Escaping Guide, The Problem, The Solution

### Community 218 - "Community 218"
Cohesion: 0.40
Nodes (5): Common Scenarios, Error #3: Incorrect Return Wrapper Format, Return Format Checklist, The Problem, The Solution

### Community 219 - "Community 219"
Cohesion: 0.40
Nodes (5): Comparison Table, Error #2: Expression Syntax Confusion, Quick Fix Guide, The Problem, The Solution

### Community 220 - "Community 220"
Cohesion: 0.40
Nodes (5): Data Access Patterns, Pattern 1: $input.all() - Most Common, Pattern 2: $input.first() - Very Common, Pattern 3: $input.item - Each Item Mode Only, Pattern 4: $node - Reference Other Nodes

### Community 221 - "Community 221"
Cohesion: 0.40
Nodes (5): Data Access Patterns, Pattern 1: _input.all() - Most Common, Pattern 2: _input.first() - Very Common, Pattern 3: _input.item - Each Item Mode Only, Pattern 4: _node - Reference Other Nodes

### Community 222 - "Community 222"
Cohesion: 0.40
Nodes (5): Generate JSON String, Handle JSON Errors, Module 1: json - JSON Operations, Parse JSON String, Pretty Print JSON

### Community 223 - "Community 223"
Cohesion: 0.40
Nodes (5): Module 6: urllib.parse - URL Operations, Parse Query String, Parse URL, URL Encode/Decode Strings, URL Encode Parameters

### Community 224 - "Community 224"
Cohesion: 0.40
Nodes (5): Module 8: random - Random Numbers, Random Choice, Random Numbers, Random Sample, Shuffle List

### Community 225 - "Community 225"
Cohesion: 0.40
Nodes (5): Cron Expression, Daily at Specific Time, Every N Minutes, Schedule Nodes, Schedule Trigger (nodes-base.scheduleTrigger)

### Community 226 - "Community 226"
Cohesion: 0.40
Nodes (5): Database Nodes, Execute Query, Insert, Postgres (nodes-base.postgres), Update

### Community 227 - "Community 227"
Cohesion: 0.40
Nodes (5): Common Node Patterns, Pattern 1: Resource/Operation Nodes, Pattern 2: HTTP-Based Nodes, Pattern 3: Database Nodes, Pattern 4: Conditional Logic Nodes

### Community 228 - "Community 228"
Cohesion: 0.40
Nodes (5): Decision Tree, Full Detail (Use Sparingly), get_node Detail Levels, Search Properties Mode, Standard Detail (DEFAULT - Use This!)

### Community 229 - "Community 229"
Cohesion: 0.40
Nodes (5): ai-friendly, minimal, runtime (RECOMMENDED), strict, Validation Profiles

### Community 230 - "Community 230"
Cohesion: 0.40
Nodes (5): Recovery Strategies, Strategy 1: Start Fresh, Strategy 2: Binary Search, Strategy 3: Clean Stale Connections, Strategy 4: Use Auto-fix

### Community 231 - "Community 231"
Cohesion: 0.40
Nodes (5): 1. Use Parameterized Queries (Prevent SQL Injection), 2. Least Privilege Access, 3. Validate Input Data, 4. Encrypt Sensitive Data, Security Best Practices

### Community 232 - "Community 232"
Cohesion: 0.40
Nodes (5): INSERT, Postgres, SELECT Query, UPDATE, UPSERT (INSERT ... ON CONFLICT)

### Community 233 - "Community 233"
Cohesion: 0.40
Nodes (5): 1. Parallel Requests, 2. Caching, 3. Conditional Fetching, 4. Batch API Calls, Performance Optimization

### Community 234 - "Community 234"
Cohesion: 0.40
Nodes (5): Error Handling Patterns, Pattern 1: Retry on Failure, Pattern 2: Fallback API, Pattern 3: Error Trigger Workflow, Pattern 4: Circuit Breaker

### Community 235 - "Community 235"
Cohesion: 0.40
Nodes (4): Pattern Structure, Real Template Examples, Scheduled Tasks Pattern, Summary

### Community 236 - "Community 236"
Cohesion: 0.40
Nodes (5): 1. Batch Processing, 2. Parallel Processing, 3. Skip if Already Running, 4. Early Exit on No Data, Performance Optimization

### Community 237 - "Community 237"
Cohesion: 0.40
Nodes (5): 1. Use Manual Trigger for Testing, 2. Test with Different Times, 3. Dry Run Mode, 4. Shorter Interval for Testing, Testing Scheduled Workflows

### Community 238 - "Community 238"
Cohesion: 0.40
Nodes (5): Deployment Phase, Implementation Phase, Planning Phase, Validation Phase, Workflow Creation Checklist

### Community 239 - "Community 239"
Cohesion: 0.40
Nodes (5): 1. Query Parameter Token, 2. Header-Based Auth, 3. Signature Verification, 4. IP Whitelist, Authentication & Security

### Community 240 - "Community 240"
Cohesion: 0.40
Nodes (5): 1. Use Manual Trigger, 2. Use curl, 3. Use Postman/Insomnia, 4. Webhook.site, Testing Webhooks

### Community 241 - "Community 241"
Cohesion: 0.40
Nodes (4): expected_behavior, id, query, skills

### Community 242 - "Community 242"
Cohesion: 0.40
Nodes (4): expected_behavior, id, query, skills

### Community 243 - "Community 243"
Cohesion: 0.40
Nodes (4): expected_behavior, id, query, skills

### Community 244 - "Community 244"
Cohesion: 0.40
Nodes (4): expected_behavior, id, query, skills

### Community 245 - "Community 245"
Cohesion: 0.40
Nodes (4): Pick a branch, Prototype, Rules that apply to both, When done

### Community 247 - "Community 247"
Cohesion: 0.40
Nodes (4): Conventions, Issue tracker: GitHub, When a skill says "fetch the relevant ticket", When a skill says "publish to the issue tracker"

### Community 248 - "Community 248"
Cohesion: 0.40
Nodes (4): Conventions, Issue tracker: GitLab, When a skill says "fetch the relevant ticket", When a skill says "publish to the issue tracker"

### Community 249 - "Community 249"
Cohesion: 0.40
Nodes (4): Conventions, Issue tracker: Local Markdown, When a skill says "fetch the relevant ticket", When a skill says "publish to the issue tracker"

### Community 250 - "Community 250"
Cohesion: 0.40
Nodes (5): computedHash, skillPath, source, sourceType, caveman

### Community 251 - "Community 251"
Cohesion: 0.40
Nodes (5): computedHash, skillPath, source, sourceType, diagnose

### Community 252 - "Community 252"
Cohesion: 0.40
Nodes (5): computedHash, skillPath, source, sourceType, grill-me

### Community 253 - "Community 253"
Cohesion: 0.40
Nodes (5): computedHash, skillPath, source, sourceType, grill-with-docs

### Community 254 - "Community 254"
Cohesion: 0.40
Nodes (5): computedHash, skillPath, source, sourceType, handoff

### Community 255 - "Community 255"
Cohesion: 0.40
Nodes (5): computedHash, skillPath, source, sourceType, improve-codebase-architecture

### Community 256 - "Community 256"
Cohesion: 0.40
Nodes (5): computedHash, skillPath, source, sourceType, n8n-code-javascript

### Community 257 - "Community 257"
Cohesion: 0.40
Nodes (5): computedHash, skillPath, source, sourceType, n8n-code-python

### Community 258 - "Community 258"
Cohesion: 0.40
Nodes (5): computedHash, skillPath, source, sourceType, n8n-expression-syntax

### Community 259 - "Community 259"
Cohesion: 0.40
Nodes (5): computedHash, skillPath, source, sourceType, n8n-mcp-tools-expert

### Community 260 - "Community 260"
Cohesion: 0.40
Nodes (5): computedHash, skillPath, source, sourceType, n8n-node-configuration

### Community 261 - "Community 261"
Cohesion: 0.40
Nodes (5): computedHash, skillPath, source, sourceType, n8n-validation-expert

### Community 262 - "Community 262"
Cohesion: 0.40
Nodes (5): computedHash, skillPath, source, sourceType, n8n-workflow-patterns

### Community 263 - "Community 263"
Cohesion: 0.40
Nodes (5): computedHash, skillPath, source, sourceType, prototype

### Community 264 - "Community 264"
Cohesion: 0.40
Nodes (5): computedHash, skillPath, source, sourceType, setup-matt-pocock-skills

### Community 265 - "Community 265"
Cohesion: 0.40
Nodes (5): tdd, computedHash, skillPath, source, sourceType

### Community 266 - "Community 266"
Cohesion: 0.40
Nodes (5): to-issues, computedHash, skillPath, source, sourceType

### Community 267 - "Community 267"
Cohesion: 0.40
Nodes (5): to-prd, computedHash, skillPath, source, sourceType

### Community 268 - "Community 268"
Cohesion: 0.40
Nodes (5): triage, computedHash, skillPath, source, sourceType

### Community 269 - "Community 269"
Cohesion: 0.40
Nodes (5): write-a-skill, computedHash, skillPath, source, sourceType

### Community 270 - "Community 270"
Cohesion: 0.40
Nodes (5): zoom-out, computedHash, skillPath, source, sourceType

### Community 271 - "Community 271"
Cohesion: 0.40
Nodes (4): expected_behavior, id, query, skills

### Community 272 - "Community 272"
Cohesion: 0.40
Nodes (4): expected_behavior, id, query, skills

### Community 273 - "Community 273"
Cohesion: 0.40
Nodes (4): expected_behavior, id, query, skills

### Community 274 - "Community 274"
Cohesion: 0.40
Nodes (4): expected_behavior, id, query, skills

### Community 275 - "Community 275"
Cohesion: 0.40
Nodes (4): expected_behavior, id, query, skills

### Community 276 - "Community 276"
Cohesion: 0.40
Nodes (4): expected_behavior, id, query, skills

### Community 277 - "Community 277"
Cohesion: 0.40
Nodes (4): expected_behavior, id, query, skills

### Community 278 - "Community 278"
Cohesion: 0.40
Nodes (4): expected_behavior, id, query, skills

### Community 279 - "Community 279"
Cohesion: 0.40
Nodes (4): expected_behavior, id, query, skills

### Community 280 - "Community 280"
Cohesion: 0.67
Nodes (3): acPost(), main(), sb

### Community 281 - "Community 281"
Cohesion: 0.50
Nodes (4): 5. Performance & Best Practices, Common Node Sequence Patterns, Optimization Tips, Success Rate Metrics

### Community 282 - "Community 282"
Cohesion: 0.50
Nodes (4): 7. Real-World Template Examples, Example 1: Scene Extraction with Error Handling, Example 2: Array to Object Property Mapping, Example 3: Binary Data Manipulation

### Community 283 - "Community 283"
Cohesion: 0.50
Nodes (4): Before Writing Skills, Document Findings, MCP Tool Testing Guidelines, Use Real Data

### Community 284 - "Community 284"
Cohesion: 0.50
Nodes (4): Branching, Commits, Git Workflow, Pull Requests

### Community 285 - "Community 285"
Cohesion: 0.50
Nodes (4): Advanced Patterns, Pattern: Conditional Node Reference, Pattern: Multi-Node Aggregation, Pattern: Pagination Handling

### Community 286 - "Community 286"
Cohesion: 0.50
Nodes (4): Checklist, Error #1: Empty Code or Missing Return Statement, The Problem, The Solution

### Community 287 - "Community 287"
Cohesion: 0.50
Nodes (4): Decision Guide, Error #6: UnsupportedFunctionError (Auth Helpers Blocked), The Problem, The Solution

### Community 288 - "Community 288"
Cohesion: 0.50
Nodes (4): Error #7: $env is not defined / Cannot access $env, The Problem, The Solution, Why This Matters

### Community 290 - "Community 290"
Cohesion: 0.50
Nodes (4): Consider Other Nodes When:, Use JavaScript When:, Use Python When:, When to Use Python vs JavaScript

### Community 291 - "Community 291"
Cohesion: 0.50
Nodes (4): Correct Node Reference Syntax, Cross-Iteration Data Not Available in Python, Production Gotchas, SplitInBatches Loop Semantics

### Community 292 - "Community 292"
Cohesion: 0.50
Nodes (4): Critical Limitation: No External Libraries, What IS Available (Standard Library), What's NOT Available, Workarounds

### Community 293 - "Community 293"
Cohesion: 0.50
Nodes (4): 1. Use Standard Library When Possible, 2. Fall Back to n8n Nodes, 3. Combine Multiple Modules, Best Practices

### Community 294 - "Community 294"
Cohesion: 0.50
Nodes (4): Available Modules, Priority 1: Most Useful (Use These), Priority 2: Moderately Useful, Priority 3: Occasionally Useful

### Community 295 - "Community 295"
Cohesion: 0.50
Nodes (4): Basic Auth Header, Decode Base64 to String, Encode String to Base64, Module 4: base64 - Encoding/Decoding

### Community 296 - "Community 296"
Cohesion: 0.50
Nodes (4): Basic Math Functions, Logarithms, Module 7: math - Mathematical Operations, Trigonometry

### Community 297 - "Community 297"
Cohesion: 0.50
Nodes (4): Data Processing (No pandas), Database Operations (No drivers), HTTP Requests (No requests library), Workarounds for Missing Libraries

### Community 298 - "Community 298"
Cohesion: 0.50
Nodes (4): Generate Unique ID, MD5 Hash, Module 5: hashlib - Hashing, SHA256 Hash

### Community 299 - "Community 299"
Cohesion: 0.50
Nodes (4): AI Agent Guide, Get Tool Documentation, Health Check, Self-Help Tools

### Community 300 - "Community 300"
Cohesion: 0.50
Nodes (4): Conversion, Critical: nodeType Formats, Format 1: Search/Validate Tools, Format 2: Workflow Tools

### Community 301 - "Community 301"
Cohesion: 0.50
Nodes (4): Deploy Template Directly, Get Template Details, Search Templates, Template Usage

### Community 302 - "Community 302"
Cohesion: 0.50
Nodes (4): Detailed Guides, Node Discovery Tools, Validation Tools, Workflow Management

### Community 303 - "Community 303"
Cohesion: 0.50
Nodes (4): Finding the Right Node, Managing Workflows, Tool Selection Guide, Validating Configuration

### Community 304 - "Community 304"
Cohesion: 0.50
Nodes (4): Pattern 1: Node Discovery (Most Common), Pattern 2: Validation Loop, Pattern 3: Workflow Editing, Tool Usage Patterns

### Community 305 - "Community 305"
Cohesion: 0.50
Nodes (4): Common Patterns from Telemetry, Pattern 1: Edit → Validate (7,841 occurrences), Pattern 2: Validate → Fix (7,266 occurrences), Pattern 3: Iterative Building (31,464 occurrences)

### Community 306 - "Community 306"
Cohesion: 0.50
Nodes (4): Delete Execution, Get Execution Details, List Executions, n8n_executions (EXECUTION MANAGEMENT)

### Community 307 - "Community 307"
Cohesion: 0.50
Nodes (4): 1. Operation-Aware Configuration, 2. Property Dependencies, 3. Progressive Discovery, Core Concepts

### Community 308 - "Community 308"
Cohesion: 0.50
Nodes (4): Configuration Anti-Patterns, ❌ Don't: Ignore Operation Context, ❌ Don't: Over-configure Upfront, ❌ Don't: Skip Validation

### Community 309 - "Community 309"
Cohesion: 0.50
Nodes (4): 1. Errors (Must Fix), 2. Warnings (Should Fix), 3. Suggestions (Optional), Error Severity Levels

### Community 310 - "Community 310"
Cohesion: 0.50
Nodes (4): 1. Conversational Agent, 2. OpenAI Functions Agent, 3. ReAct Agent, Agent Types

### Community 311 - "Community 311"
Cohesion: 0.50
Nodes (4): Advanced Patterns, Fallback Language Models, RAG (Retrieval-Augmented Generation), Streaming Responses

### Community 312 - "Community 312"
Cohesion: 0.50
Nodes (4): Buffer Memory, Memory Configuration, Summary Memory, Window Buffer Memory

### Community 313 - "Community 313"
Cohesion: 0.50
Nodes (4): Error Handling, Pattern 1: Tool Execution Errors, Pattern 2: LLM API Errors, Pattern 3: Invalid Tool Outputs

### Community 314 - "Community 314"
Cohesion: 0.50
Nodes (4): Example (Customer Support), Example (Data Analyst), Prompt Engineering for Agents, System Prompt Structure

### Community 315 - "Community 315"
Cohesion: 0.50
Nodes (4): Aggregation, Data Transformation, Data Type Conversions, Schema Mapping

### Community 316 - "Community 316"
Cohesion: 0.50
Nodes (4): Error Handling, Pattern 1: Check Rows Affected, Pattern 2: Constraint Violations, Pattern 3: Rollback on Error

### Community 317 - "Community 317"
Cohesion: 0.50
Nodes (4): Pattern 1: BEGIN/COMMIT/ROLLBACK, Pattern 2: Atomic Operations, Pattern 3: Error Rollback, Transaction Handling

### Community 318 - "Community 318"
Cohesion: 0.50
Nodes (4): Pattern 1: Wait Between Requests, Pattern 2: Exponential Backoff, Pattern 3: Respect Rate Limit Headers, Rate Limiting

### Community 319 - "Community 319"
Cohesion: 0.50
Nodes (4): Advanced Patterns, Conditional Execution, Dependent Schedules, Dynamic Scheduling

### Community 320 - "Community 320"
Cohesion: 0.50
Nodes (4): Common Timezones, Handle Daylight Saving, Set Workflow Timezone, Timezone Considerations

### Community 321 - "Community 321"
Cohesion: 0.50
Nodes (4): Cron Mode (Advanced), Days & Hours Mode, Interval Mode, Schedule Configuration

### Community 322 - "Community 322"
Cohesion: 0.50
Nodes (4): Error Handling, Pattern 1: Error Trigger Workflow, Pattern 2: Retry with Backoff, Pattern 3: Partial Failure Handling

### Community 323 - "Community 323"
Cohesion: 0.50
Nodes (4): Monitoring & Logging, Pattern 1: Execution Log Table, Pattern 2: Metrics Collection, Pattern 3: Summary Notifications

### Community 324 - "Community 324"
Cohesion: 0.50
Nodes (4): Bidirectional Threshold Checking, Google Drive, Google Sheets, Integration-Specific Gotchas

### Community 325 - "Community 325"
Cohesion: 0.50
Nodes (4): Error Handling, Pattern 1: Try-Catch with Error Trigger, Pattern 2: Validation Early Exit, Pattern 3: Continue On Fail

### Community 326 - "Community 326"
Cohesion: 0.50
Nodes (4): High Volume, Large Payloads, Performance Considerations, Retries

### Community 327 - "Community 327"
Cohesion: 0.83
Nodes (3): capture(), step(), hitl-loop.template.sh script

### Community 328 - "Community 328"
Cohesion: 0.50
Nodes (3): Bad Tests, Good and Bad Tests, Good Tests

### Community 331 - "Community 331"
Cohesion: 0.67
Nodes (3): Automated Testing, Manual Testing, Testing

### Community 332 - "Community 332"
Cohesion: 0.67
Nodes (3): Bad Evaluations, Evaluation Guidelines, Good Evaluations

### Community 333 - "Community 333"
Cohesion: 0.67
Nodes (3): Code Standards, JSON (Evaluations), Markdown

### Community 334 - "Community 334"
Cohesion: 0.67
Nodes (3): Common Pitfalls, ✅ Do, ❌ Don't

### Community 335 - "Community 335"
Cohesion: 0.67
Nodes (3): Cross-References, Documentation Standards, SKILL.md Frontmatter

### Community 336 - "Community 336"
Cohesion: 0.67
Nodes (3): Evaluations, File Naming Conventions, Skills

### Community 337 - "Community 337"
Cohesion: 0.67
Nodes (3): Getting Help, Reporting Bugs, Support

### Community 341 - "Community 341"
Cohesion: 0.67
Nodes (3): Additional Resources, n8n Documentation, Related Files

### Community 342 - "Community 342"
Cohesion: 0.67
Nodes (3): Correct Return Formats, Incorrect Return Formats, Return Format Requirements

### Community 343 - "Community 343"
Cohesion: 0.67
Nodes (3): Mode Selection Guide, Run Once for All Items (Recommended - Default), Run Once for Each Item

### Community 344 - "Community 344"
Cohesion: 0.67
Nodes (3): Additional Resources, n8n Documentation, Related Files

### Community 345 - "Community 345"
Cohesion: 0.67
Nodes (3): Correct Return Formats, Incorrect Return Formats, Return Format Requirements

### Community 346 - "Community 346"
Cohesion: 0.67
Nodes (3): Mode Selection Guide, Run Once for All Items (Recommended - Default), Run Once for Each Item

### Community 347 - "Community 347"
Cohesion: 0.67
Nodes (3): Python (Beta) - Recommended, Python Modes: Beta vs Native, Python (Native) (Beta)

### Community 348 - "Community 348"
Cohesion: 0.67
Nodes (3): Best Practices, Do, Don't

### Community 349 - "Community 349"
Cohesion: 0.67
Nodes (3): get_node (Unified Node Information), Unified Tool Reference, validate_node (Unified Validation)

### Community 350 - "Community 350"
Cohesion: 0.67
Nodes (3): Best Practices, Do, Don't

### Community 351 - "Community 351"
Cohesion: 0.67
Nodes (3): Best Practices, Do, ❌ Don't

### Community 352 - "Community 352"
Cohesion: 0.67
Nodes (3): Configuration Workflow, Example: Configuring HTTP Request, Standard Process

### Community 353 - "Community 353"
Cohesion: 0.67
Nodes (3): Example: HTTP Request Body, Example: IF Node singleValue, Handling Conditional Requirements

### Community 354 - "Community 354"
Cohesion: 0.67
Nodes (3): Best Practices, ✅ Do, ❌ Don't

### Community 355 - "Community 355"
Cohesion: 0.67
Nodes (3): Accessing Different Parts, Standard Structure, Webhook Data Structure

### Community 381 - "Community 381"
Cohesion: 0.20
Nodes (9): Auth bug fixed (commits `4004298`, `b90fef3`), Current project state, Cómo usar el grafo de conocimiento, Graphify installed (commits `fa96904`, `e964cdc`), Handoff — Antigravity / TimeTrack, Incidente de infraestructura (resuelto), Si el backend vuelve a dar 502/530, Suggested skills (+1 more)

### Community 382 - "Community 382"
Cohesion: 0.25
Nodes (7): Fix, Pendiente (no aplicado aún), Problema original, Qué NO cambia el fix, Requisito de infraestructura: `N8N_WEBHOOK_SECRET` en ambos servicios, Resiliencia del listener SignalR (2N Access Commander), Verificar tras un redeploy

## Knowledge Gaps
- **2480 isolated node(s):** `name`, `version`, `private`, `type`, `dev` (+2475 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **19 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `supabase` connect `Community 40` to `Community 33`, `Community 71`, `Community 39`, `Community 48`, `Community 49`, `Community 212`, `Community 52`?**
  _High betweenness centrality (0.002) - this node is a cross-community bridge._
- **Why does `skills` connect `Community 356` to `Community 256`, `Community 257`, `Community 258`, `Community 259`, `Community 260`, `Community 261`, `Community 262`, `Community 263`, `Community 264`, `Community 265`, `Community 266`, `Community 267`, `Community 268`, `Community 269`, `Community 270`, `Community 250`, `Community 251`, `Community 252`, `Community 253`, `Community 254`, `Community 255`?**
  _High betweenness centrality (0.002) - this node is a cross-community bridge._
- **Why does `getSupabaseAdmin()` connect `Community 47` to `Community 35`, `Community 7`, `Community 41`, `Community 43`, `Community 19`, `Community 20`, `Community 24`?**
  _High betweenness centrality (0.001) - this node is a cross-community bridge._
- **What connects `name`, `version`, `private` to the rest of the system?**
  _2480 weakly-connected nodes found - possible documentation gaps or missing edges._
- **Should `Community 0` be split into smaller, more focused modules?**
  _Cohesion score 0.04081632653061224 - nodes in this community are weakly interconnected._
- **Should `Community 1` be split into smaller, more focused modules?**
  _Cohesion score 0.0425531914893617 - nodes in this community are weakly interconnected._
- **Should `Community 2` be split into smaller, more focused modules?**
  _Cohesion score 0.043478260869565216 - nodes in this community are weakly interconnected._